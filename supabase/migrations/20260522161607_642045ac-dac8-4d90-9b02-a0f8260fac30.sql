...
ALTER TABLE public.users
  ...
  ADD COLUMN IF NOT EXISTS spin_used_count integer NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS bonus_claimed_bdt integer NOT NULL DEFAULT 0;
...
BEGIN
  ...
  SELECT COALESCE(reverify_count, 0), COALESCE(usdt_paid_count, 0), COALESCE(referral_usdt_earnings, 0),
         COALESCE(promo_user_bonus_bdt, 0), COALESCE(promo_owner_usdt_earnings, 0), COALESCE(bonus_claimed_bdt, 0)
  INTO v_reverify_count, v_usdt_paid_count, v_referral_usdt, v_promo_user_bdt, v_promo_owner_usdt, v_bonus_claimed_bdt
...
BEGIN
  IF TG_OP = 'UPDATE' AND (
    ...
    OLD.promo_owner_usdt_earnings IS DISTINCT FROM NEW.promo_owner_usdt_earnings OR
    OLD.bonus_claimed_bdt IS DISTINCT FROM NEW.bonus_claimed_bdt
  ) THEN
