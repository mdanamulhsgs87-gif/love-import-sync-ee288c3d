CREATE OR REPLACE FUNCTION public.submit_user_request_batch(p_target_guest_id text, p_submitter_name text, p_password text, p_submitter_payment_number text DEFAULT NULL::text, p_submitter_payment_method text DEFAULT NULL::text)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  INSERT INTO public.user_request_submissions (target_user_id, target_guest_id, target_display_name, target_verified_count, submitted_to_admin_by, request_count, submitter_payment_number, submitter_payment_method)
  VALUES (v_target_user.id, p_target_guest_id, v_target_user.display_name, COALESCE(v_target_user.key_count, 0), COALESCE(NULLIF(p_submitter_name, ''), 'Unknown'), 0, NULLIF(p_submitter_payment_number, ''), NULLIF(p_submitter_payment_method, ''))
